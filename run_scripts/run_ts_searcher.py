...
def obtain_transition_states(
    ...
    - num_workers (int): Number of CPU workers to use for calculations (only used if batch is False and there's
        no available GPU, setting to 0 will allocate all visible CPUs)
    ...
    for rxn_id, reaction in reaction_list:
        ...
        ts_optimizers.append(
            TSOptimizer(path_generator, reactive_complex_factors, attempts, save_paths, results_directory, calc_hess)
        )
