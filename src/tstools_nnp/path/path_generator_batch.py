...
from tstools_nnp.utils.batch_calculations import FIRE, ConstrainedCalculator, GeometryCalculation, HookeanConstraint
...
class PathGeneratorBatch:
    ...
    def __init__(self, path_generators, nnp_calculator, device):
        ...
        self.device = device
        self.pad = False
        if isinstance(self.nnp_calculator, AIMNET):
            self.pad = True
    ...
    def get_paths(self, max_iterations=100):
        ...
        done_states = {PathGenerator.OPT_STATES["completed"], PathGenerator.OPT_STATES["failed"]}
        while not all(state in done_states for state in self.states.values()):
            if max_iterations == 0:
                break
    ...
    def initialize_optimizations(self):
        ...
        opt_dict = {}
        for path_generator in self.path_generators:
            if path_generator.opt_state in (PathGenerator.OPT_STATES["failed"], PathGenerator.OPT_STATES["completed"]):
                continue
        ...
        for i in range(len(opt_results[0])):
            ...
            for j in range(len(opt_results)):
                if j == 0:
                    ...
                    curr = opt_results[j][i].detach().numpy()
                    prev = opt_results[j - 1][i].detach().numpy()
                    if not np.allclose(curr, prev, atol=1e-8):
                        trajectory.append(self.unpad_coords(opt_results[j][i].detach().numpy()))
    ...
    def update_states(self, trajectories):
        ...
        path_generators = [
            pg
            for pg in self.path_generators
            if pg.opt_state not in (PathGenerator.OPT_STATES["completed"], PathGenerator.OPT_STATES["failed"])
        ]
