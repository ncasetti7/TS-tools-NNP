...
class TSOptimizerBatch:
    ...
    def generate_ts_batch(self):
        ...
        for i in range(len(self.ts_optimizers[0].reactive_complex_factors)):
            path_generators = [
                ts_optimizer.path_generator
                for ts_optimizer in self.ts_optimizers
                if ts_optimizer.path_generator.rxn_id not in success_list
            ]
            [
                path_generator.set_reactive_complex_factor(ts_optimizer.reactive_complex_factors[i])
                for path_generator, ts_optimizer in zip(path_generators, self.ts_optimizers)
            ]
            ...
            for ts_optimizer in self.ts_optimizers:
                if ts_optimizer.path_generator.rxn_id in energies:
                    ...
                    if ts_optimizer.save_paths:
                        rxn_id = ts_optimizer.path_generator.rxn_id
                        rc_factor = ts_optimizer.path_generator.reactive_complex_factor
                        cheminformatics.path_to_xyz_file(
                            paths[rxn_id],
                            ts_optimizer.path_generator.atomic_symbols,
                            f"path_dir/path_{rc_factor}_{index}.xyz",
                        )
                    success = ts_optimizer.check_ts_guesses(
                        energies[ts_optimizer.path_generator.rxn_id],
